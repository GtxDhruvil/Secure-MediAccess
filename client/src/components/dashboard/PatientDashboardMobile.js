...
const OTPModal = ({ show, onClose, onSubmit, otp, setOtp }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="bg-slate-800/80 border border-slate-700 w-full max-w-sm mx-4 rounded-xl shadow-lg p-6 animate-slideUp">
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3 sm:justify-end">
          ...
          <button onClick={onSubmit} className="px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700 transition-colors w-full sm:w-auto">Approve Access</button>
        </div>
      </div>
    </div>
  );
...
const PatientDashboard = () => {
  ...
  const renderOverview = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-8">
      <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg p-4 sm:p-6 border-l-4 border-blue-400 card-glow">
        <div className="flex items-center">
          <div className="p-2 bg-blue-900/50 rounded-lg">
            <FileText className="h-5 w-5 sm:h-6 sm:w-6 text-blue-300" />
          </div>
          <div className="ml-3 sm:ml-4">
            <p className="text-xs sm:text-sm font-medium text-slate-300">Total Records</p>
            <p className="text-xl sm:text-2xl font-bold text-white">{stats.totalRecords}</p>
          </div>
        </div>
      </div>

      <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg p-4 sm:p-6 border-l-4 border-yellow-400 card-glow">
        <div className="flex items-center">
          <div className="p-2 bg-yellow-900/50 rounded-lg">
            <Clock className="h-5 w-5 sm:h-6 sm:w-6 text-yellow-300" />
          </div>
          <div className="ml-3 sm:ml-4">
            <p className="text-xs sm:text-sm font-medium text-slate-300">Pending Requests</p>
            <p className="text-xl sm:text-2xl font-bold text-white">{stats.pendingRequests}</p>
          </div>
        </div>
      </div>

      <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg p-4 sm:p-6 border-l-4 border-green-400 card-glow">
        <div className="flex items-center">
          <div className="p-2 bg-green-900/50 rounded-lg">
            <Unlock className="h-5 w-5 sm:h-6 sm:w-6 text-green-300" />
          </div>
          <div className="ml-3 sm:ml-4">
            <p className="text-xs sm:text-sm font-medium text-slate-300">Active Access</p>
            <p className="text-xl sm:text-2xl font-bold text-white">{stats.activeAccess}</p>
          </div>
        </div>
      </div>

      <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg p-4 sm:p-6 border-l-4 border-purple-400 card-glow">
        <div className="flex items-center">
          <div className="p-2 bg-purple-900/50 rounded-lg">
            <Calendar className="h-5 w-5 sm:h-6 sm:w-6 text-purple-300" />
          </div>
          <div className="ml-3 sm:ml-4">
            <p className="text-xs sm:text-sm font-medium text-slate-300">Last Login</p>
  ...
  const renderMedicalRecords = () => (
    <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg shadow-lg overflow-hidden">
      <div className="px-4 sm:px-6 py-4 border-b border-slate-700">
        <h3 className="text-base sm:text-lg font-medium text-white">Medical Records</h3>
      </div>
        ...
        <table className="min-w-full divide-y divide-slate-700">
          <thead className="bg-slate-800/70">
            <tr>
              ...
              <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider hidden sm:table-cell">
                Doctor
              </th>
              <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider hidden md:table-cell">
                Type
              </th>
              <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider hidden lg:table-cell">
                Date
              </th>
              <th className="px-3 sm:px-6 py-3 text-left text-xs font-medium text-slate-300 uppercase tracking-wider">
                Status
              ...
              <tr key={record.id} className={`${index % 2 === 0 ? 'bg-slate-900/50' : 'bg-slate-800/50'} hover:bg-slate-700/50 transition-colors duration-200`}>
                <td className="px-3 sm:px-6 py-4">
                  <div>
                    ...
                    <div className="text-xs sm:text-sm text-slate-400">{record.description}</div>
                    {/* Mobile-only additional info */}
                    <div className="sm:hidden mt-2 space-y-1">
                      {record.doctor && (
                        <div className="text-xs text-slate-400">
                          <span className="font-medium">Dr.</span> {record.doctor.firstName} {record.doctor.lastName}
                        </div>
                      )}
                      <div className="text-xs text-slate-400">
                      ...
                      <div className="text-xs text-slate-400">
                        <span className="font-medium">Date:</span> {format(new Date(record.recordDate), 'MMM dd, yyyy')}
                      </div>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-3 sm:px-6 py-4 hidden lg:table-cell text-sm text-white">
  ...
  const renderAccessRequests = () => (
    <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg shadow-lg overflow-hidden">
      <div className="px-4 sm:px-6 py-4 border-b border-slate-700">
        <h3 className="text-base sm:text-lg font-medium text-white">Access Requests</h3>
      </div>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-700">
          <thead className="bg-slate-800/70">
              ...
              <tr key={request.id} className={`${index % 2 === 0 ? 'bg-slate-900/50' : 'bg-slate-800/50'} hover:bg-slate-700/50 transition-colors duration-200`}>
                <td className="px-3 sm:px-6 py-4">
                  <div>
                    ...
                    <div className="text-xs sm:text-sm text-slate-400">{request.doctor.specialization}</div>
                    {/* Mobile-only additional info */}
                    <div className="sm:hidden mt-1">
                      <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-purple-900/70 text-purple-300 border border-purple-700">
                        {request.requestType}
                      </span>
                    </div>
                  </div>
                  ...
                  {request.status === 'pending' ? (
                    <div className="flex flex-col sm:flex-row gap-2">
                      <button
                        ...
                        className="px-3 py-1 rounded-md bg-green-600 text-white text-xs hover:bg-green-700 transition-colors w-full sm:w-auto"
                      >
                        Approve
                      </button>
                      <button
                        onClick={() => handleDenyClick(request.id)}
                        className="px-3 py-1 rounded-md bg-red-600 text-white text-xs hover:bg-red-700 transition-colors w-full sm:w-auto"
                      >
                        Deny
                      </button>
                    </div>
  ...
  const renderProfile = () => (
    <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg shadow-lg p-4 sm:p-6">
      <h3 className="text-base sm:text-lg font-medium text-white mb-4 sm:mb-6">Profile Information</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
        ...
        <div>
          <label className="block text-xs sm:text-sm font-medium text-slate-300">Phone Number</label>
          <p className="mt-1 text-sm text-white">{user.phoneNumber}</p>
        </div>
        <div>
          <label className="block text-xs sm:text-sm font-medium text-slate-300">Date of Birth</label>
          <p className="mt-1 text-sm text-white">
            {user.dateOfBirth ? format(new Date(user.dateOfBirth), 'MMM dd, yyyy') : 'N/A'}
          </p>
        </div>
        <div>
          <label className="block text-xs sm:text-sm font-medium text-slate-300">Gender</label>
  ...
  const renderSecurity = () => (
    <div className="bg-slate-900/70 backdrop-blur-sm border border-slate-700 rounded-lg shadow-lg p-4 sm:p-6">
      <h3 className="text-base sm:text-lg font-medium text-white mb-4 sm:mb-6">Security Settings</h3>
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-lg gap-3">
          <div>
          ...
          <button className="bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-colors duration-200 w-full sm:w-auto">
            Enable
          </button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-lg gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400">Manage active sessions and devices</p>
          </div>
          <button className="bg-slate-600 text-white px-4 py-2 rounded-md hover:bg-slate-700 transition-colors duration-200 w-full sm:w-auto">
            Manage
          </button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 bg-slate-800/50 border border-slate-700 rounded-lg gap-3">
          <div>
            ...
            <p className="text-xs sm:text-sm text-slate-400">Control who can access your medical records</p>
          </div>
          <button className="bg-slate-600 text-white px-4 py-2 rounded-md hover:bg-slate-700 transition-colors duration-200 w-full sm:w-auto">
            Configure
          </button>
  ...
  return (
    <div className="min-h-screen bg-transparent">
      ...
      <header className="hidden md:block bg-slate-900/70 backdrop-blur-sm border-b border-slate-700 shadow-lg sticky top-0 z-40">
      ...
      <nav className="hidden md:block bg-slate-800/60 backdrop-blur-sm border-b border-slate-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
